...
public class RepDao {
	public static boolean getExcelForORDERCODE(HttpServletResponse response,String code_start,String code_end,String ordercode,String ename)  {
		Page<Record>page =RepDao.OrderCode(1, 1000000000, code_start, code_end,ordercode,ename);
      ...
      //首行 样式2
       ...
       cellStyle2.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 居中    
		...
		for(int i=0;i<wllist.size();i++){
			...
			cell4.setCellValue(r.getStr("ename"));
			HSSFCell cell5 = row.createCell((short) 5);
			cell5.setCellStyle(cellStyle1);
			cell5.setCellValue(r.getStr("price"));
			HSSFCell cell6 = row.createCell((short) 6);
			cell6.setCellStyle(cellStyle1);
			cell6.setCellValue(r.getStr("differ"));
			HSSFCell cell7 = row.createCell((short) 7);
			cell7.setCellStyle(cellStyle1);
			cell7.setCellValue(r.getStr("type"));
			HSSFCell cell8 = row.createCell((short) 8);
			cell8.setCellStyle(cellStyle1);
			cell8.setCellValue(r.getStr("fname"));
		}
	 ...
	 * 每单盈亏
	 */
	public static Page<Record>OrderCode(int pageno, int pagesize,String code_start,String code_end,String ordercode,String ename){
		 String select = " SELECT c.fname,case c.type when 1 then '双品' when 2 then '多品' when 3 then '送花'  when 4 then '节日' when 5 then '定制'  when 6 then '闪购'  when 7 then '主题'  when 8 then '拼团' when 9 then '玫瑰套餐' end as type ,"
		 		+ "a.ename, a.ordercode,FORMAT(IFNULL(a.price,0),2) as price,a.code,FORMAT(IFNULL(sum(e.price*d.num),0),2) as price2,FORMAT(IFNULL(f.price/f.cycle,0),2) as price1,FORMAT(IFNULL(f.price/f.cycle,0)-IFNULL(sum(e.price*d.num),0)-IFNULL(a.price,0),2) as differ";
	...
	public static String getSumPrice(String code_start,String code_end,String ordercode,String ename) {
		 String sqlColumnSelect = "select FORMAT(sum(a.sumprice),2) as sumprice ";
			 String sqlExceptionSelect = " from "
			 		+ " (SELECT a.code,a.ordercode,a.ename,(IFNULL(f.price/f.cycle,0)-IFNULL(sum(e.price*d.num),0)-IFNULL(a.price,0)) as sumprice  "
			 		+ " from "
			 		+ " f_order_info as a"
			 		+ " LEFT join f_order_pro as b on a.id = b.oid"
			 		+ " LEFT JOIN f_product c on b.pid = c.id"
			 		...
			 		+ " LEFT JOIN (select IFNULL(sum(price*totalNum)/sum(num*totalNum),0) price, code,fid from f_purchasem  group by fid,code) e on d.fid = e.fid and a.code = e.code "
			 		+ " LEFT JOIN f_order as f on a.ordercode = f.ordercode"
			 		+ " WHERE  b.`type`=0 GROUP by a.code,a.ordercode order by a.code desc) AS a"
			 		+ " where 1=1";
			 if (StrKit.notBlank(code_start)) {
				 sqlExceptionSelect+=String.format(" and a.code >= '%s'", code_start);
			 }
			 if (StrKit.notBlank(code_end)) {
				 sqlExceptionSelect+=String.format(" and a.code <= '%s'", code_end);
				 }
			 if (StrKit.notBlank(ordercode)) {
				 sqlExceptionSelect+=String.format(" and a.ordercode = '%s'", ordercode);
		     }
			 if (StrKit.notBlank(ename)) {
				 sqlExceptionSelect+=String.format(" and a.ename = '%s'", ename);
		     }
			 String sumprice=Db.queryStr(sqlColumnSelect+sqlExceptionSelect);
			 return sumprice;
	}
